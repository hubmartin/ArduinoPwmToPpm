...
ISR( WDT_vect ) // If watchdog is triggered then enable missing signal flag and copy power on or failsafe positions
{
    ...
    if ( ppm_generator_active || brownout_reset )
    {
        ...
        for ( uint8_t i = 0; i < PPM_ARRAY_MAX; i++ ) { ppm[ i ] = failsafe_ppm[ i ]; } } 
    ...
    {   
     ...
     // if (( servo_pin == 8  )  && (  ppm_generator_active = true) ) ppm_stop(); 
        ...
        else
        {
            ...
            #define FAILCENTRE 1 
            #ifdef FAILCENTRE servo_width = failsafe_ppm[ _ppm_channel ]; 
            // failsafe defaults, most channels centred, throttle lowered. 
            #endif 
            
            #ifdef FAILHOLD
            servo_width = ppm[ _ppm_channel ]; 
            // all channels hold their previous position! 
            #endif
            CHECK_PINS_NOERROR: 
